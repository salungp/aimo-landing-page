/**
 * ARCHIVED — the original MarketsTogether: a full-bleed scene pinned to the
 * centre of the screen on a 150vh sticky track, playing the portrait Lottie
 * pre-render on mobile and the 1948x1129 Lottie on tablet up.
 *
 * Superseded on 2026-09-17 by MarketsTogether.tsx, which follows OneBalance's
 * two-column frame (illustration one side, copy the other). Kept verbatim so
 * the pinned scene can be restored: copy this file over ../MarketsTogether.tsx,
 * rename the export back to `MarketsTogether`, and fix the two-level relative
 * imports. Its media is still in the repo — /lottie/markets-together-desktop.json
 * and /video/markets-together-mobile.{mp4,webp}.
 */

/* Container and TitleReveal are only used by the commented-out caption block
 * below; they stay imported so restoring it is a matter of deleting the
 * comment markers. */
...
export default function MarketsTogetherScrollScene() {
  return (
