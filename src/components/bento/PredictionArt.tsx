...
function buildLine(values: number[], map: (p: number) => number) {
...
function buildArea(values: number[], map: (p: number) => number) {
  const bottom = Math.max(...values.map(map)) + 12;
  const last = (values.length - 1) * STEP;
  return `${buildLine(values, map)} L${last} ${bottom} L0 ${bottom} Z`;
}
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      >
        ...
        <div ref={trackRef} className="absolute inset-0" style={{ willChange: "transform" }}>
          ...
          >
            ...
            <path d={buildArea(yes, yesY)} fill="url(#bento-yes-fill)" />
            <path d={buildArea(no, yesY)} fill="url(#bento-no-fill)" />
            <path
              d={buildLine(yes, yesY)}
              stroke="#8B5CF6"
              strokeWidth="1.39683"
              strokeLinecap="round"
      ...
      {(
        ...
        <div key={head.color} className="pointer-events-none absolute top-[14px] left-0" style={{ width: 254 }}>
          <span
